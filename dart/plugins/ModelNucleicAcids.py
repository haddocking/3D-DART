...
def PluginCore(paramdict, inputlist):
    ...
    if not paramdict['stats'] is None:
...
class CommandlineOptionParser:
    ...
    def CommandlineOptionParser(self):
        ...
        parser.add_option("-n", "--name", action="store", dest="name", type="string", default='parfile',
                          help="Supply a name for the output parameter file")
        parser.add_option("-a", "--automodel", action="store_true", dest="automodel", default=False,
                          help="Run automodel process")
        parser.add_option("-g", "--gltolerance", action="store", dest="gltolerance", type="float", default=1.0,
                          help="Deviation from ensamble average for global conformation, scale 0-1>")
        parser.add_option("-b", "--glvariance", action="store", dest="glvariance", type="float", default=0.8,
                          help="Deviation from optimal base-pair and base-pair step parameter for global conformation, scale 0-1>")
        parser.add_option("-i", "--glsmoothing", action="store", dest="glsmoothing", type="float", default=0.8,
                          help="Smoothing for twist, roll and tilt, scale 0-1")
        parser.add_option("-j", "--lcvariance", action="store", dest="lcvariance", type="float", default=0.8,
                          help="Deviation from optimal base-pair and base-pair step parameter for local conformation, scale 0-1>")
        parser.add_option("-c", "--number", action="store", dest="number", type="int", default=10,
                          help="Define the number of structures for the automatic modeling process to generate")
        parser.add_option("-r", "--refbp", action="store", dest="refbp", type="float",
                          help="Define the reference base-pair over whitch to bend. default is the middel most base-pair")
        parser.add_option("-o", "--minangle", action="store", dest="minangle",
                          help="Define either the minimum bend angle or give a custom sequence of bend angles")
        parser.add_option("-p", "--maxangle", action="store", dest="maxangle", type="float",
                          help="Define maximum bend angle")
        parser.add_option("-q", "--anglestep", action="store", dest="anglestep", type="float",
                          help="Define bend-angle step size")
        parser.add_option("-d", "--startbp", action="store", dest="startbp", type="int",
                          help="Define start base-pair for bend")
        parser.add_option("-e", "--endbp", action="store", dest="endbp", type="int",
                          help="Define end base-pair for bend")
        parser.add_option("-k", "--minorient", action="store", dest="minorient",
                          help="Define either the minimum bend angle in space or give a custom sequence of bend angles")
        parser.add_option("-l", "--maxorient", action="store", dest="maxorient", type="float",
                          help="Define maximum angle of bend in space")
        parser.add_option("-m", "--orientstep", action="store", dest="orientstep", type="float",
                          help="Define step size of bend angle in space")
        parser.add_option("-v", "--verbose", action="store_true", dest="verbose", default=False,
                          help="Output to standard out")
        parser.add_option("-s", "--bpstep", action="store", dest="bpstep", default="None",
                          help="Supply custom values for base-pair step parameters as a komma seperated list. shift,slide,rise,tilt,roll,twist")
        parser.add_option("-t", "--bp", action="store", dest="bp", default="None",
                          help="Supply custom values for base-pair parameters as a komma seperated list.")
        parser.add_option("-u", "--mingroove", action="store", dest="mingroove", type="float",
                          help="Define minimum major groove width")
        parser.add_option("-w", "--maxgroove", action="store", dest="maxgroove", type="float",
                          help="Define maximum major groove width")
        parser.add_option("-x", "--groovestep", action="store", dest="groovestep", type="float",
                          help="Define major groove width step size")
        parser.add_option("-y", "--groovestart", action="store", dest="groovestart", type="int",
                          help="Define start base-pair for major groove width change")
        parser.add_option("-z", "--grooveend", action="store", dest="grooveend", type="int",
                          help="Define end base-pair for major groove width change")
        parser.add_option("--helicalphase", action="store", dest="helicalphase", type="float",
                          help="Set the DNA helical phasing (bp/turn)")
...
class ModelNucleicAcids:
    ...
    def _MakeDecisions(self, minp=None, maxp=None, pstep=None, def1=None, def2=None):
        ...
        if orgminp == None and orgmaxp == None:
            ...
            if len(orgminp) == 1:
                ...
                tmpmaxp.append(def1)
                tmppstep.append(None)
                print("    * ERROR: Custom sequence for", minp, "not equal to sequence length. Set to default of",
                      tmpminp[0])
        else:
            if len(orgminp) == 1:
                tmpminp = orgminp
                ...
                if orgpstep == None:
                    ...
                    tmppstep = orgpstep
                print(
                "    * Custom sequence for", minp, " set to:", tmpminp, "Value for", maxp, "set to:", tmpmaxp[0])
                print("    * Value for", pstep, "set to:", tmppstep[0])
                print(
                "    * The values in the custom sequence will be increased", tmppstep[0], "times with a value of:",
                tmpmaxp[0])
            else:
                tmpminp.append(def1)
                tmpmaxp.append(def1)
    ...
    def _MakeArray(self, minl=None, maxl=None, step=None, nondiv=False):
        ...
        if nondiv == False:
            if len(minl) == 1:
                tmparray = []
                if minl[0] == maxl[0]:
                    listl = [minl[0] / (self.paramdict['nrbp'] + self.paramdict['nrbp'] / 10)] * self.paramdict[
                        'nrbp']
    ...
    def _LimitManualModels(self, models):
        ...
        if float(self.paramdict['number']) > float(
                MAXMODELS):  # Amount of models that can be generated is not allowed to exeed an maximum
            self.paramdict['number'] = MAXMODELS  # Maximum is defined in Constants.py (MAXMODELS)
            print(
                        "    * WARNING: number of requested models exceeds allowed maximum of %s. Set to maximum" % MAXMODELS)
        else:
            print("    * Number of models to generate set to: %i" % self.paramdict['number'])

        if float(len(models)) / float(self.paramdict[
                                          'number']) < 1.0:  # Amount of models that can be generated cannot be more than number of combinations
            ...
            print("    * Take a representative selection of %i models from a total of %i models" % (
            len(modelrange), len(models)))
            for n in modelrange:
                tmp.append(models[n])
            return tmp
    ...
    def _LimitModels(self, anglem, orientm):
        ...
        models = []
        if float(len(anglem) * len(orientm)) / float(self.paramdict[
                                                         'number']) < 1.0:  # Amount of models that can be generated cannot be more than number of combinations
            ...
            print("    * Take a representative selection of %i models from a total of %i models" % (
            self.paramdict['number'], len(anglem) * len(orientm)))
            for a in anglerange:
    ...
    def ReadOut(self, files):
        ...
        for lines in bpsteplines:
            ...
            for value in range(6):
                lib[value + 7].append(transform_dash(lines[value]))
    ...
    def ReadMultiout(self, files):
        ...
        for lines in bpsteplines:
            ...
            for value in range(12):
                lib[value + 14].append(transform_dash(lines[value]))
    ...
    def CheckParamdict(self):
        ...
        if self.paramdict['startbp'] == None:
            ...
            if self.paramdict['startbp'] > len(self.baseparameter['sequence']):
                self.paramdict['startbp'] = int(1)
                print(
                "    * WARNING: Start base-pair for bending is larger that sequence length. Start base-pair is set to first base-pair:",
                self.paramdict['startbp'])
            else:
        ...
        if self.paramdict['groovestart'] == None:
            ...
            print("    * No start base-pair for major groove width set, use first base-pair:",
                  self.paramdict['groovestart'])
        else:
            if self.paramdict['groovestart'] > len(self.baseparameter['sequence']):
                self.paramdict['groovestart'] = int(1)
                print(
                "    * WARNING: Start base-pair for major groove width is larger that sequence length. Start base-pair is set to first base-pair:",
                self.paramdict['groovestart'])
            else:
        ...
        if self.paramdict['endbp'] == None:
            ...
            if self.paramdict['endbp'] > len(self.baseparameter['sequence']):
                ...
                print(
                "    * WARNING: End base-pair for bending is larger that sequence length. End base-pair is set to last base-pair:",
                self.endbp)
            elif self.paramdict['endbp'] < self.paramdict['startbp']:
                self.paramdict['endbp'] = int(len(self.baseparameter['sequence']))
                print(
                "    * WARNING: End base-pair for bending cannot be smaller than the start base-pair. End base-pair set to last base-pair:",
                self.endbp)
            else:
        ...
        if self.paramdict['grooveend'] == None:
            ...
            print(
            "    * No end base-pair for major groove width set, use last base-pair:", self.paramdict['grooveend'])
        else:
            if self.paramdict['grooveend'] > len(self.baseparameter['sequence']):
                self.paramdict['grooveend'] = int(len(self.baseparameter['sequence']))
                print(
                "    * WARNING: End base-pair for groove width is larger that sequence length. End base-pair is set to last base-pair:",
                self.paramdict['grooveend'])
            elif self.paramdict['grooveend'] < self.paramdict['groovestart']:
                self.paramdict['grooveend'] = int(len(self.baseparameter['sequence']))
                print(
                "    * WARNING: End base-pair for major groove width cannot be smaller than the start base-pair. End base-pair set to last base-pair:",
                self.paramdict['grooveend'])
            else:
        ...
        if self.paramdict['automodel'] == False:
            if self.paramdict['refbp'] == None:
                ...
                print(
                "    * No value for the reference base-pair used for bending supplied. Set to middle of sequence:",
                self.paramdict['refbp'])
            elif self.paramdict['refbp'] < self.paramdict['startbp'] or self.paramdict['refbp'] > self.paramdict[
                'endbp']:
                self.paramdict['refbp'] = float((self.paramdict['nrbp'] / 2) + self.paramdict['startbp'] - 1)
                print(
                "    * WARNING: Reference base-pair has to reside between the start and end base-pair. Set to middle of sequence:",
                self.paramdict['refbp'])
            else:
        ...
        if self.paramdict['automodel'] == True:
            if self.paramdict['refbp'] == None and self.bendref['refbp'] == None:
                ...
                print(
                "    * Reference base-pair extracted from multibend.stat file set to:", self.paramdict['refbp'])
            elif self.paramdict['refbp'] < self.paramdict['startbp'] or self.paramdict['refbp'] > self.paramdict[
                'endbp']:
                self.paramdict['refbp'] = float((self.paramdict['nrbp'] / 2) + self.paramdict['startbp'] - 1)
                print(
        ...
        if not self.paramdict['mingroove'] == None:
            if float(self.paramdict['mingroove']) < 14.0 or float(self.paramdict['mingroove']) > 19.0:
                print("    * Allowed range for major groove width (14.0-19.0 A) violated width value:",
                      self.paramdict['mingroove'])
        ...
        if self.paramdict['automodel'] == False:
            self.paramdict['minangle'], self.paramdict['maxangle'], self.paramdict[
                'anglestep'] = self._MakeDecisions(minp='minangle', maxp='maxangle', pstep='anglestep', def1=None,
                                                   def2=1.0)
            self.paramdict['minorient'], self.paramdict['maxorient'], self.paramdict[
                'orientstep'] = self._MakeDecisions(minp='minorient', maxp='maxorient', pstep='orientstep',
                                                    def1=45.0, def2=1.0)
        if self.paramdict['automodel'] == True:
            if self.paramdict['minangle'] == None and self.paramdict['maxangle'] == None:
                if self.paramdict['gltolerance'] == 0.0:
                    ...
                    self.paramdict['minangle'] = [((self.bendref['globbend'][0] - self.bendref['globbendsd'][0]) *
                                                   self.paramdict['gltolerance'])]
                    self.paramdict['maxangle'] = [((self.bendref['globbend'][0] + self.bendref['globbendsd'][0]) *
                                                   self.paramdict['gltolerance'])]
                    self.paramdict['anglestep'] = [
                        (self.paramdict['maxangle'][0] - self.paramdict['minangle'][0]) / self.paramdict['number']]
                print(
                    "    * No bend-angle preferences supplied. Set automaticly based on range extracted from multibend.stat file. Settings:")
                print("      Minimum bend-angle: %1.2f, maximum bend-angle: %1.2f, angle step: %1.2f" % (
                self.paramdict['minangle'][0],
                self.paramdict['maxangle'][0], self.paramdict['anglestep'][0]))
            else:
                print(
                    "    * User supplied angle preferences will override automatic defined values from multibend.stat file")
                self.paramdict['minangle'], self.paramdict['maxangle'], self.paramdict[
                    'anglestep'] = self._MakeDecisions(minp='minangle', maxp='maxangle', pstep='anglestep',
                                                       def1=float(0), def2=1.0)
            if self.paramdict['minorient'] == None and self.paramdict['maxorient'] == None:
                if self.paramdict['gltolerance'] == 0.0:
                    ...
                    self.paramdict['orientstep'] = [
                        (self.paramdict['maxorient'][0] - self.paramdict['minorient'][0]) / self.paramdict[
                            'number']]
                print(
                    "    * No bend-angle orientation preferences supplied. Set automaticly based on range extracted from multibend.stat file. Settings:")
                print(
                            "      Minimum bend-angle orientation: %1.2f, maximum bend-angle orientation: %1.2f, orientation step: %1.2f" % (
                    self.paramdict['minorient'][0],
                    self.paramdict['maxorient'][0], self.paramdict['orientstep'][0]))
            else:
                print(
                    "    * User supplied angle-orientation preferences will override automatic defined values from multibend.stat file")
                self.paramdict['minorient'], self.paramdict['maxorient'], self.paramdict[
                    'orientstep'] = self._MakeDecisions(minp='minorient', maxp='maxorient', pstep='orientstep',
                                                        def1=float(45), def2=1.0)
                ...
                for param in xrange(len(self.paramdict['bpstep'])):
                    try:
                        if len(self.paramdict['bpstep'][param]) > 0:
                            ...
                            print("    * Custom value for base-pair step parameter %s set to %1.2f" % (
                            BASEPAIR_STEPS[param], self.paramdict[BASEPAIR_STEPS[param]]))
                    except:
                        pass
            else:
                ...
                for param in xrange(len(self.paramdict['bp'])):
                    try:
                        if len(self.paramdict['bp'][param]) > 0:
                            ...
                            print("    * Custom value for base-pair step parameter %s set to %1.2f" % (
                            BASEPAIRS[param], self.paramdict[BASEPAIRS[param]]))
                    except:
                        pass
            else:
    ...
    def Automodel(self):
        ...
        if self.paramdict['base'] == None:
            print(
                "    * WARNING: No base parameter file supplied, try to aquire from sequence in base statistics file")
            ...
            if os.path.isfile('base.pdb'):
                ...
                print(
                    "    * ERROR: failed to aquire base parameter file from sequence in base statistics file. Stopping")
                sys.exit(0)
    ...
    def Manualmodel(self):
        ...
        print("--> Start generation of parameter files")
        models = []
        if 'slidematrix' in self.paramdict:
        ...
        for model in models:
            ...
            if not model[0][0] == None:
                ...
                self.swapdatabase.Update('roll',
                                         self._AdjustParams(newparam=locroll, oldparam=self.swapdatabase['roll']))
                self.swapdatabase.Update('tilt',
                                         self._AdjustParams(newparam=loctilt, oldparam=self.swapdatabase['tilt']))
...
if __name__ == '__main__':
    ...
    if paramdict['input'] == None:
        print(
            "    * Please supply .par and/or multibend.stat, multiout.stat file using option -b or use option -h/--help for usage")
        sys.exit(0)
    else:
