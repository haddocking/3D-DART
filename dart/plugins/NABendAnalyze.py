...
class MeasureBend:
	...
	def _GlobRollTilt(self, tilt, roll):
		...
		if frup == 0:
			...
			gltiltarr = (tiltarr*np.cos(ctwistarr*p))+(rollarr*np.sin(ctwistarr*p))
			glrollarr = (tiltarr*-np.sin(ctwistarr*p))+(rollarr*np.cos(ctwistarr*p))
	...
	def _ListFraction(self, v1,v2,v3):
		...
		b = np.array(v2)
		c = np.array(v3)
		d = (np.power(v2,2))/(np.power(v1,2))
		e = (np.power(v3,2))/(np.power(v1,2))
		f = d.tolist()
		g = e.tolist()
		return f,g
	...
	def _CalculateCore(self, files):
		...
		self.sdgroll = np.std(self.glroll[self.corr_zone[0]:self.corr_zone[1]])
		self.sdgtilt = np.std(self.gltilt[self.corr_zone[0]:self.corr_zone[1]])
		self.sdftilt = np.std(self.ftilt[self.corr_zone[0]:self.corr_zone[1]])
		self.sdfroll = np.std(self.froll[self.corr_zone[0]:self.corr_zone[1]])
		self.sdbpangle = np.std(self.bpangle[self.corr_zone[0]:self.corr_zone[1]])
		self.sdctwist = np.std(self.ctwist[self.corr_zone[0]:self.corr_zone[1]])
		...
		self.sdorient = np.std(self.orient[self.corr_zone[0]:self.corr_zone[1]])
	...
	def _MultiCalculate(self):
		...
		for n in range(len(self.roll)):
			if len(self.roll[n]) > 1:
				stroll.append(np.std(self.roll[n]))
				sttilt.append(np.std(self.tilt[n]))
				sttwist.append(np.std(self.twist[n]))
			else:
		...
		"""Calculate standard deviation of all summed parameters"""
		self.sdgroll = np.std(self.glroll[0][self.corr_zone[0]:self.corr_zone[1]])
		self.sdgtilt = np.std(self.gltilt[0][self.corr_zone[0]:self.corr_zone[1]])
		self.sdftilt = np.std(self.ftilt[0][self.corr_zone[0]:self.corr_zone[1]])
		self.sdfroll = np.std(self.froll[0][self.corr_zone[0]:self.corr_zone[1]])
		self.sdbpangle = np.std(self.bpangle[0][self.corr_zone[0]:self.corr_zone[1]])
		self.sdorient = np.std(self.orient[0][self.corr_zone[0]:self.corr_zone[1]])
		self.sdctwist = np.std(self.ctwist[0][self.corr_zone[0]:self.corr_zone[1]])
