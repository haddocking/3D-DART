        ...
        for atom1 in i:
		  ...
		  if (not (atom1.name[0]=="H") and (not atom2.name[0]=="H")):
		     d=calc_dist(atom1.position,atom2.position)
       	             if (d<cutoff):
		      if (molecule1.id,i.name,i.number,molecule2.id,p.name,p.number) not in dict:
...
class AnaNBcontacts:
	...
	def MatchPaires(self):
		...
		while count < len(pairlist):
			...
			if (instance[0],instance[1],instance[2],instance[3],instance[4],instance[5]) in self.pairdict:
				pass
			else:
				index = []
				i = 0
